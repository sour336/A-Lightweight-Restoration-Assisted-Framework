...
from model.osrnetT import OSRNetT
...
def compute_loss(db320, db240, db160, batch, istrain):
    ...
    loss_mse = 0
    loss_mse += mse(db320, batch['label320'])
    ...
    loss += loss1 + loss2 + 0.001*loss3 #没归一化就是0.02
    # 峰值信噪比(PSNR)
    psnr = 10 * torch.log(MAX_DIFF ** 2 / loss_mse) / log10
    return {'total': loss, 'psnr': psnr, 'mse':loss_mse, 'FFT':loss3}
...
if __name__ == "__main__":
    ...
    # 3.网络
    net = torch.nn.DataParallel(OSRNetT(xavier_init_all=config.net['xavier_init_all'])).cuda()
    # 如果使用之前已训练的网络参数
    ...
    for epoch in tqdm(range(config.train['num_epochs']), file=sys.stdout, desc=str(config.train['num_epochs'])+' epochs'):
        # 根据当前 epoch 设置学习率
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            # 得到网络预测结果
            _, _, _, _, _, _, db320, db240, db160 = net(batch['img320'], batch['img240'], batch['img160'],
                                                                          batch['mask320_t'], batch['mask240_t'], batch['mask160_t'])
            # 计算损失
            loss = compute_loss(db320, db240, db160, batch, istrain=True)
        ...
        if first_val or epoch % config.train['log_epoch'] == config.train['log_epoch'] - 1:
            ...
            with torch.no_grad():
                for step, batch in enumerate(val_dataloader):
                    ...
                    _, _, _, _, _, _, db320, db240, db160 = net(batch['img320'], batch['img240'], batch['img160'],
                                                               batch['mask320_t'], batch['mask240_t'], batch['mask160_t'])
                    loss = compute_loss(db320, db240, db160, batch, istrain=False)
                    for k in loss:
                        loss[k] = float(loss[k].cpu().detach().numpy())
                ...
                torch.save(net.state_dict(), './checkpoints/' + str(epoch) + '.pth')
